...
namespace
{
  ...
  void RefreshExecutionHash(FCrowdTargetRegionQuotaExecutionState& State)
  {
  ...
  bool PointInsideExpanded(const FVector2f& Point,
    const FCrowdSharedFlowObstacleSpec& Obstacle, const float Expansion)
  {
  ...
  bool SegmentSafe(const FVector2f& Start, const FVector2f& End,
    const FCrowdSharedFlowFieldConfig& FlowConfig, const float Clearance,
  ...
  const FCrowdTargetPolarCell* FindCell(
    const FCrowdTargetPolarTopology& Topology, const int32 CellKey)
  {
    ...
    const FCrowdTargetPolarTopology& Topology)
  {
    ...
    {
      const FCrowdSharedFlowSample Sample = FCrowdSharedFlowFieldKernel::Sample(
...
int32 FCrowdTargetRegionTransportKernel::SectorCountForRadius(const float RadiusCm)
{
...
FVector2f FCrowdTargetRegionTransportKernel::ComposeTargetAdvectedFarFlowVelocity(
...
FVector2f FCrowdTargetRegionTransportKernel::ComposeEngagedHoldVelocity(
...
FCrowdTargetEngagementDecision
FCrowdTargetRegionTransportKernel::ResolveTargetEngagement(
  const ECrowdTargetDistanceResponsePolicy Policy,
...
{
  FCrowdTargetEngagementDecision Result;
...
int32 FCrowdTargetRegionTransportKernel::ComputeEdgeSoftClearancePenaltyCm(
...
uint32 FCrowdTargetRegionTransportKernel::ComputeFeasibleGraphHash(
  const FCrowdTargetPolarTopology& Topology)
{
  uint32 Hash = Fold(FnvOffset, 2);
  for (const auto& Cell : Topology.Cells)
...
void FCrowdTargetRegionTransportKernel::BuildTopology(
  const FCrowdTargetRegionTransportSettings& Settings,
  const FCrowdSharedFlowFieldConfig& FlowConfig,
  ...
  const FCrowdSharedFlowField* SharedFlowField)
{
  ...
  FCrowdSharedFlowFieldConfig StableFlow = FlowConfig;
    ...
    for (int32 Sector = 0; Sector < SectorCount; ++Sector)
    {
      ...
      if (SharedFlowField && SharedFlowField->IsValid())
      {
        const FCrowdSharedFlowSample Sample =
          FCrowdSharedFlowFieldKernel::Sample(*SharedFlowField,
            FVector(Cell.WorldAnchorCm.X, Cell.WorldAnchorCm.Y, 0.0f));
        Cell.bNavigationBlocked =
          Sample.Status != ECrowdFlowLocationStatus::Reachable;
      }
      Cell.bFeasible = !Cell.bTargetBlocked && !Cell.bBoundsBlocked
        && !Cell.bObstacleBlocked && !Cell.bNavigationBlocked;
      ...
      OutSummary.NavigationBlockedCellCount += Cell.bNavigationBlocked ? 1 : 0;
  ...
  auto AddEdge = [&](const FCrowdTargetPolarCell& From,
    const FCrowdTargetPolarCell& To, const bool bCrossBand)
  {
...
void FCrowdTargetRegionTransportKernel::BuildDemand(
  const TConstArrayView<FCrowdTargetRegionTransportAgent> InputAgents,
  const FCrowdTargetRegionTransportSettings& Settings,
  const FCrowdSharedFlowFieldConfig& FlowConfig,
  const FCrowdSharedFlowField* SharedFlowField,
  const FCrowdTargetPolarTopology& Topology,
  FCrowdTargetRegionDemandResult& OutDemand,
  const TConstArrayView<FCrowdTargetRegionTransportAgent> InputExternalAgents)
{
  OutDemand = {};
  if (!Topology.bValid || Settings.DemandRegionCount <= 0) return;
  TArray<FCrowdTargetRegionTransportAgent> Agents(InputAgents);
  Agents.Sort([](const auto& A, const auto& B) { return A.AgentId < B.AgentId; });
  TArray<FCrowdTargetRegionTransportAgent> ExternalAgents(InputExternalAgents);
  ...
  for (const auto& ExternalAgent : ExternalAgents)
  {
    ...
    const FCrowdTargetPolarCell* Cell = FindCell(Topology, DirectCell);
  ...
  for (const FCrowdTargetDemandRegion& Region : OutDemand.Regions)
  ...
  for (FCrowdTargetDemandRegion& Region : OutDemand.Regions)
  {
  ...
  for (FCrowdTargetRegionAgentDemandState& State : OutDemand.AgentStates)
  {
  ...
  for (const FCrowdTargetPolarEdge& Edge : Topology.Edges)
...
void FCrowdTargetRegionTransportKernel::UpdateStaticDemandPopulation(
  const TConstArrayView<FCrowdTargetRegionTransportAgent> InputAgents,
  const FCrowdTargetRegionTransportSettings& Settings,
  const FCrowdSharedFlowFieldConfig& FlowConfig,
  const FCrowdSharedFlowField* SharedFlowField,
...
{
  // Capacity admission depends on the complete stable membership and current
  // terminal occupancy. Rebuild the deterministic demand snapshot so static
  // updates cannot retain stale overflow or admission facts.
  BuildDemand(InputAgents, Settings, FlowConfig, SharedFlowField, Topology,
    InOutDemand, InputExternalAgents);
  return;
}

void FCrowdTargetRegionTransportKernel::SolveTransport(
  const FCrowdTargetPolarTopology& Topology,
  ...
  const TConstArrayView<FCrowdTargetRegionQuotaAgentClaim> ReservedClaims)
{
  OutPlan = {};
  ...
  TArray<FCrowdTargetRegionQuotaAgentClaim> StableReservedClaims(ReservedClaims);
  ...
  TMap<int64, FCrowdTargetPolarEdgeFlow> FlowByKey;
...
void FCrowdTargetRegionTransportKernel::ValidatePlanForDemand(
  const FCrowdTargetPolarTopology& Topology,
  const FCrowdTargetRegionDemandResult& Demand,
  ...
  FCrowdTargetRegionPlanValidationResult& OutValidation)
{
  ...
  for (const auto& Flow : Plan.EdgeFlows)
  {
    ...
    const FCrowdTargetPolarEdge* Found = nullptr;
    for (const auto& Edge : Topology.Edges)
...
void FCrowdTargetRegionTransportKernel::InitializeQuotaExecutionState(
  const FCrowdTargetRegionFlowPlan& Plan,
  FCrowdTargetRegionQuotaExecutionState& OutState)
{
  ...
  for (const FCrowdTargetPolarEdgeFlow& Flow : Plan.EdgeFlows)
  {
    FCrowdTargetRegionQuotaEdgeState& Edge = OutState.Edges.AddDefaulted_GetRef();
...
void FCrowdTargetRegionTransportKernel::ReplacePlanPreservingClaims(
  const FCrowdTargetPolarTopology& Topology,
  const FCrowdTargetRegionDemandResult& Demand,
  const FCrowdTargetRegionFlowPlan& PreviousPlan,
  ...
  FCrowdTargetRegionPlanReplacementSummary& OutSummary)
{
  ...
  OutExecution = {};
  OutSummary = {};
  TArray<FCrowdTargetRegionQuotaAgentClaim> StableClaims =
  ...
  TArray<FCrowdTargetRegionQuotaAgentClaim> EligibleClaims;
  ...
  FCrowdTargetRegionPlanValidationResult ExecutionValidation;
  while (true)
  {
...
void FCrowdTargetRegionTransportKernel::ValidateQuotaExecutionState(
  const FCrowdTargetPolarTopology& Topology,
  const FCrowdTargetRegionDemandResult& Demand,
  const FCrowdTargetRegionFlowPlan& Plan,
  ...
  FCrowdTargetRegionPlanValidationResult& OutValidation)
{
  ...
  for (int32 Index = 0; Index < State.Edges.Num(); ++Index)
  {
    ...
    const FCrowdTargetPolarEdgeFlow* PlanEdge = Plan.EdgeFlows.FindByPredicate(
      [&EdgeState](const auto& Flow)
      {
        return Flow.FromCellKey == EdgeState.FromCellKey
          && Flow.ToCellKey == EdgeState.ToCellKey;
      });
    const FCrowdTargetPolarEdge* TopologyEdge = Topology.Edges.FindByPredicate(
  ...
  TMap<int32, const FCrowdTargetRegionAgentDemandState*> DemandByAgent;
  ...
  TArray<FCrowdTargetRegionAgentDemandState> StableDemandStates(Demand.AgentStates);
  ...
  for (const FCrowdTargetRegionAgentDemandState& AgentState :
    Demand.AgentStates)
  ...
  for (int32 EdgeIndex = 0; EdgeIndex < State.Edges.Num(); ++EdgeIndex)
  {
    const FCrowdTargetRegionQuotaEdgeState& Edge = State.Edges[EdgeIndex];
  ...
  for (const FCrowdTargetRegionQuotaAgentClaim& Claim : State.ActiveClaims)
  {
    const auto* const* AgentStatePtr = DemandByAgent.Find(Claim.AgentId);
    const FCrowdTargetRegionAgentDemandState* AgentState =
...
void FCrowdTargetRegionTransportKernel::BuildGuidanceWithExecution(
  const TConstArrayView<FCrowdTargetRegionTransportAgent> InputAgents,
  const FCrowdTargetRegionTransportSettings& Settings,
  const FCrowdTargetPolarTopology& Topology,
  const FCrowdTargetRegionDemandResult& Demand,
  ...
  const int32 ShardEntityCount)
{
  ...
  TArray<FCrowdTargetRegionTransportAgent> Agents(InputAgents);
  Agents.Sort([](const auto& A, const auto& B) { return A.AgentId < B.AgentId; });
  TMap<int32, const FCrowdTargetRegionAgentDemandState*> StateByAgent;
  for (const auto& State : Demand.AgentStates) StateByAgent.Add(State.AgentId, &State);
  TArray<FCrowdTargetPolarEdgeFlow> StableFlows(Plan.EdgeFlows);
  StableFlows.Sort([](const FCrowdTargetPolarEdgeFlow& A,
    const FCrowdTargetPolarEdgeFlow& B)
  {
    return A.FromCellKey != B.FromCellKey ? A.FromCellKey < B.FromCellKey
      : A.ToCellKey < B.ToCellKey;
  });
  TMap<int32, TArray<const FCrowdTargetPolarEdgeFlow*>> Outgoing;
  for (const auto& Flow : StableFlows)
    Outgoing.FindOrAdd(Flow.FromCellKey).Add(&Flow);
  for (auto& Pair : Outgoing)
    Pair.Value.Sort([](const FCrowdTargetPolarEdgeFlow& A,
      const FCrowdTargetPolarEdgeFlow& B) { return A.ToCellKey < B.ToCellKey; });
  FCrowdTargetRegionQuotaExecutionState WorkState = InOutExecutionState;
  ...
  TArray<FCrowdTargetRegionQuotaAgentClaim> ReconciledClaims;
    ...
    ShardBegin += StableShardEntityCount)
  {
      ...
      AgentIndex < ShardEnd; ++AgentIndex)
    {
      ...
      {
        Result.Mode = ECrowdTargetRegionGuidanceMode::CapacityHold;
      ...
      {
        Result.Mode = ECrowdTargetRegionGuidanceMode::FarFlow;
      ...
      else if (State->bEngagedHold)
      {
        ...
        Result.Mode = ECrowdTargetRegionGuidanceMode::EngagedHold;
      ...
      {
        Result.Mode = ECrowdTargetRegionGuidanceMode::TerminalSettle;
        const FVector2f Offset = Agent.Location - Settings.TargetLocation;
      ...
      {
        const TArray<const FCrowdTargetPolarEdgeFlow*>* Flows = Outgoing.Find(State->CurrentCellKey);
        const FCrowdTargetPolarEdgeFlow* Chosen = nullptr;
        if (const int32* ClaimIndex = ClaimIndexByAgent.Find(Agent.AgentId))
        {
          ...
          for (const auto* Flow : *Flows)
          {
            ...
            if (EdgeIndex && AvailableByEdge[*EdgeIndex] > 0)
            {
              ...
              Chosen = Flow;
              FCrowdTargetRegionQuotaAgentClaim& NewClaim =
        ...
        {
          Result.Mode = ECrowdTargetRegionGuidanceMode::Transport;
          Result.NextCellKey = Chosen->ToCellKey;
          const FCrowdTargetPolarCell& ToCell = Topology.Cells[Chosen->ToCellKey];
  ...
  OutSummary.FirstUnroutedCellKey = INDEX_NONE;
  for (const FCrowdTargetRegionGuidanceResult& Result : OutResults)
  {
    switch (Result.Mode)
    {
    case ECrowdTargetRegionGuidanceMode::FarFlow: ++OutSummary.FarFlowAgentCount; break;
    case ECrowdTargetRegionGuidanceMode::Transport: ++OutSummary.TransportAgentCount; break;
    case ECrowdTargetRegionGuidanceMode::TerminalSettle:
      ++OutSummary.TerminalSettleAgentCount; break;
    case ECrowdTargetRegionGuidanceMode::EngagedHold:
      ++OutSummary.EngagedHoldAgentCount; break;
    case ECrowdTargetRegionGuidanceMode::CapacityHold:
      ++OutSummary.CapacityHoldAgentCount; break;
    case ECrowdTargetRegionGuidanceMode::Unrouted:
      ++OutSummary.UnroutedAgentCount;
      if (OutSummary.FirstUnroutedAgentId == INDEX_NONE)
      {
  ...
  for (const FCrowdTargetRegionAgentDemandState& AgentState :
    ...
    if (AgentState.bSupply
      && AdmissionQuotaByCell.IsValidIndex(AgentState.CurrentCellKey))
      ++AdmissionQuotaByCell[AgentState.CurrentCellKey];
  for (const FCrowdTargetRegionQuotaEdgeState& Edge : WorkState.Edges)
  {
  ...
  for (const auto& Flow : StableFlows)
  {
    FCrowdTargetRegionGuidanceConsumption Consumption;
...
void FCrowdTargetRegionTransportKernel::BuildGuidance(
  const TConstArrayView<FCrowdTargetRegionTransportAgent> InputAgents,
  const FCrowdTargetRegionTransportSettings& Settings,
  const FCrowdTargetPolarTopology& Topology,
  const FCrowdTargetRegionDemandResult& Demand,
  ...
  FCrowdTargetRegionGuidanceSummary& OutSummary)
{
  FCrowdTargetRegionQuotaExecutionState ExecutionState;
