...
bool Satisfies(
  ...
  const TConstArrayView<FCrowdVelocityHalfPlane> HalfPlanes,
  const float Epsilon,
  const float VelocityScale)
{
  const FNumericalTolerances Tolerances = ComputeTolerances(VelocityScale);
  for (const FCrowdVelocityHalfPlane& HalfPlane : HalfPlanes)
  {
...
bool ClipLineInterval(
  const FVector2f LinePoint,
  const FVector2f LineDirection,
  const FCrowdVelocityHalfPlane& HalfPlane,
  const float Epsilon,
  float& InOutMinimumT,
  float& InOutMaximumT,
  FCrowdVelocityHalfPlaneNumericalSummary* OutSummary)
{
  ...
  FCrowdVelocityHalfPlaneNumericalSummary* OutSummary)
{
  ...
  const FCrowdVelocityHalfPlane& Active = Input.HalfPlanes[ActiveIndex];
  ...
  FVector2f& OutVelocity)
{
  ...
  {
    const FCrowdVelocityHalfPlane& HalfPlane = Input.HalfPlanes[Index];
    ...
    {
      const FCrowdVelocityHalfPlane& Other = Input.HalfPlanes[OtherIndex];
...
bool FCrowdVelocityHalfPlaneKernel::CanonicalizeInput(
  const FCrowdVelocityHalfPlaneInput& Input,
  FCrowdVelocityHalfPlaneInput& OutCanonical)
{
  ...
  if (!FMath::IsFinite(Input.PreferredVelocity.X)
    ...
    return false;
  for (const FCrowdVelocityHalfPlane& HalfPlane : OutCanonical.HalfPlanes)
  {
  ...
  OutCanonical.HalfPlanes.Sort([](
    const FCrowdVelocityHalfPlane& A,
    const FCrowdVelocityHalfPlane& B)
  {
...
bool FCrowdVelocityHalfPlaneKernel::SolveContinuous(
  const FCrowdVelocityHalfPlaneInput& Input,
  FVector2f& OutVelocity,
  FCrowdVelocityHalfPlaneNumericalSummary* OutNumericalSummary)
{
  FCrowdVelocityHalfPlaneInput Canonical;
  ...
  {
    const FCrowdVelocityHalfPlane& HalfPlane = Canonical.HalfPlanes[ConstraintIndex];
...
bool FCrowdVelocityHalfPlaneKernel::ValidateVelocity(
  const FCrowdVelocityHalfPlaneInput& Input,
...
ECrowdVelocityQuantizationResult FCrowdVelocityHalfPlaneKernel::QuantizeAndValidate(
  const FCrowdVelocityHalfPlaneInput& Input,
  ...
  FVector2f& OutVelocity)
{
  FCrowdVelocityHalfPlaneInput Canonical;
  if (!CanonicalizeInput(Input, Canonical))
    return ECrowdVelocityQuantizationResult::NoSolution;
  const auto& Settings = Canonical.Settings;
  const FVector2f Preferred = ClampSpeed(
    Quantize(Canonical.PreferredVelocity, Settings.VelocityQuantumCmps),
    Settings.MaxSpeedCmps);
  ...
  {
    OutVelocity = Rounded;
    return ECrowdVelocityQuantizationResult::CenterFeasible;
  }
  if (FindNeighborhoodQuantized(Canonical, ContinuousVelocity, Preferred, OutVelocity))
    return ECrowdVelocityQuantizationResult::NeighborhoodRecovered;
  if (FindGeometryQuantized(Canonical, ContinuousVelocity, Preferred, OutVelocity))
    return ECrowdVelocityQuantizationResult::GeometryRecovered;
  return ECrowdVelocityQuantizationResult::NoSolution;
}

FCrowdVelocityHalfPlaneResult FCrowdVelocityHalfPlaneKernel::Solve(
  const FCrowdVelocityHalfPlaneInput& Input,
  FCrowdVelocityHalfPlaneNumericalSummary* OutNumericalSummary)
{
  FCrowdVelocityHalfPlaneResult Result;
  FCrowdVelocityHalfPlaneInput Canonical;
  ...
  if (InsideSpeedCircle(Canonical.PreferredVelocity, Settings.MaxSpeedCmps,
    ...
    && Satisfies(Canonical.PreferredVelocity, Canonical.HalfPlanes,
      Settings.BehaviorEpsilonCmps, Settings.MaxSpeedCmps))
  {
    Result.Status = ECrowdVelocityHalfPlaneSolveStatus::PreferredFeasible;
  ...
  {
    Result.Status = ECrowdVelocityHalfPlaneSolveStatus::ExactFeasible;
    Result.bContinuousValid = ValidateVelocity(Canonical, Result.ContinuousVelocity);
    if (!Result.bContinuousValid)
      Result.Status = ECrowdVelocityHalfPlaneSolveStatus::NumericalFailure;
  }
  else
  {
    Result.Status = ECrowdVelocityHalfPlaneSolveStatus::ProvenInfeasible;
    return Result;
  }
  Result.Quantization = QuantizeAndValidate(
    Canonical, Result.ContinuousVelocity, Result.QuantizedVelocity);
  Result.bQuantizedValid = Result.Quantization != ECrowdVelocityQuantizationResult::NoSolution
