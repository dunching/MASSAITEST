  ...
  uint32 Fold(uint32 Hash, const int32 Value)
  {
    for (int32 Byte = 0; Byte < 4; ++Byte)
    {
      Hash ^= static_cast<uint8>(
        (static_cast<uint32>(Value) >> (Byte * 8)) & 0xffu);
      Hash *= FnvPrime;
    }
    return Hash;
  }
...
void FCrowdFacingKernel::Resolve(
  const TConstArrayView<FCrowdFacingInput> InputView,
  ...
  FCrowdFacingSummary& OutSummary)
{
  ...
  TArray<FCrowdFacingInput> Inputs(InputView);
  ...
  for (const FCrowdFacingInput& Input : Inputs)
  {
    ...
    FCrowdFacingResult& Result = OutSummary.Results.AddDefaulted_GetRef();
    ...
    Hash = Fold(Hash, FMath::RoundToInt(
      Result.DesiredYawDegrees / Settings.AngleQuantumDegrees));
    Hash = Fold(Hash, FMath::RoundToInt(
      Result.ResolvedYawDegrees / Settings.AngleQuantumDegrees));
