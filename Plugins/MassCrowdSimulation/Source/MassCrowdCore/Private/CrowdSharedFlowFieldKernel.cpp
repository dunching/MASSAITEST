...
namespace
{
  ...
  bool IsSegmentClear(
    ...
    const FVector& End)
  {
    for (const FCrowdSharedFlowObstacleSpec& Obstacle : Config.ObstacleSpecs)
    {
...
void FCrowdSharedFlowField::Reset()
{
  Config = FCrowdSharedFlowFieldConfig();
...
bool FCrowdSharedFlowFieldKernel::CanTraverseCellEdge(
...
bool FCrowdSharedFlowFieldKernel::CanTraverseWorldSegment(
  const FCrowdSharedFlowFieldConfig& Config,
  const FVector& Start,
  const FVector& End)
{
  return CanTraverseWorldSegmentCompatible(Config, Start, End);
}
...
namespace
{
  ...
  uint64 MakeCenterNodeKey(const int32 Version, const int32 CellKey)
  {
    ...
    Hash = FoldNavigationKey(Hash,
      static_cast<int32>(ECrowdNavigationNodeKind::CenterAnchor));
    return FoldNavigationKey(Hash, CellKey);
  }

  uint64 MakeConnectionNodeKey(
    const int32 Version,
    const ECrowdNavigationNodeKind Kind,
  ...
  FVector NavigationNodeLocation(
    const FCrowdSharedFlowField& Field,
    const FCrowdNavigationNode& Node)
  {
    return FVector(
    ...
    const bool bRequireReachable)
  {
    ...
    for (int32 Ring = 0; Ring <= MaxRing && Candidates.IsEmpty(); ++Ring)
    {
      ...
      for (const int32 NodeIndex : RingNodeIndices)
      {
        ...
        if (!FCrowdSharedFlowFieldKernel::CanTraverseWorldSegment(
  ...
  void BuildConnectionIntervals(
    ...
    TArray<FCrowdNavigationSafeInterval>& OutIntervals,
    TArray<FCrowdNavigationNode>& OutNodes)
  {
    ...
    const FVector ContractMax = FVector(Config.BoundsMax)
      - FVector(Config.AgentInflateCm, Config.AgentInflateCm, 0.0f);
    const bool bVertical = Kind == ECrowdNavigationNodeKind::VerticalEdgeConnection;
    ...
    auto FlushRun = [&](const int32 RunEnd)
    {
      ...
      FCrowdNavigationSafeInterval Interval;
      Interval.Kind = Kind;
  ...
  bool BuildV2NavigationGraph(FCrowdSharedFlowField& Field)
  {
    ...
    for (int32 Cell = 0; Cell < CellCount; ++Cell)
    {
      ...
      Node.Kind = ECrowdNavigationNodeKind::CenterAnchor;
      ...
      for (int32 X = 0; X < Field.Width; ++X)
      {
        ...
        if (X + 1 < Field.Width)
        {
          BuildConnectionIntervals(Field,
            ECrowdNavigationNodeKind::VerticalEdgeConnection,
            Cell, Cell + 1,
  ...
  uint32 HashV2Topology(const FCrowdSharedFlowField& Field)
  {
    uint32 Hash = 2166136261u;
...
bool FCrowdSharedFlowField::IsValid() const
{
...
int32 FCrowdSharedFlowField::LocationToCellIndex(const FVector& Location) const
{
...
FVector FCrowdSharedFlowField::CellCenter(const int32 CellIndex) const
{
...
FCrowdSharedFlowFieldConfig FCrowdSharedFlowFieldKernel::MakeSf1Config(const int32 Revision)
{
  ...
  {
    FCrowdSharedFlowObstacleSpec& Spec = Config.ObstacleSpecs.AddDefaulted_GetRef();
    Spec.ObstacleId = Id;
    Spec.Center = Center;
    Spec.Extent = Extent;
  };
...
bool FCrowdSharedFlowFieldKernel::Build(
  const FCrowdSharedFlowFieldConfig& Config,
  FCrowdSharedFlowField& OutField)
{
  ...
  if (Config.ConnectivityContractVersion > 0)
  {
    ...
    TArray<FCrowdSharedFlowObstacleSpec> SortedObstacles = Config.ObstacleSpecs;
...
bool FCrowdSharedFlowFieldKernel::BuildTopology(
  const FCrowdSharedFlowFieldConfig& Config,
  FCrowdSharedFlowField& OutField)
{
  OutField.Reset();
...
bool FCrowdSharedFlowFieldKernel::ResolveGoalAnchor(
  const FCrowdSharedFlowField& Field,
...
bool FCrowdSharedFlowFieldKernel::BuildIntegrationForAnchor(
  ...
  FCrowdSharedFlowField& Field)
{
  ...
  {
    if (Field.NavigationIntegrationCost[NodeIndex] == InfiniteCost) continue;
    const FCrowdNavigationNode& Node = Field.NavigationNodes[NodeIndex];
    const int32 Cells[2] = {Node.PrimaryCellKey, Node.SecondaryCellKey};
    for (const int32 Cell : Cells)
    {
      if (!Field.IntegrationCost.IsValidIndex(Cell)) continue;
...
FCrowdSharedFlowSample FCrowdSharedFlowFieldKernel::Sample(
  const FCrowdSharedFlowField& Field,
  const FVector& Location)
{
  FCrowdSharedFlowSample Result;
  ...
  {
    Result.Status = ECrowdFlowLocationStatus::OutOfBounds;
    return Result;
  }
  if (Field.Config.ConnectivityContractVersion >= 2)
  {
    ...
    Result.Status = Result.bBlocked
      ? ECrowdFlowLocationStatus::BlockedRasterCell
      : (Result.bUnreachable
        ? ECrowdFlowLocationStatus::UnreachableFreeCell
    ...
    if (Attachments.IsEmpty())
    {
      ...
      if (!Result.bBlocked)
        Result.Status = ECrowdFlowLocationStatus::UnreachableFreeCell;
      return Result;
    }
    ...
    Result.Status = ECrowdFlowLocationStatus::Reachable;
  ...
  const FCrowdReachableFlowCellSearchResult Recovery =
...
FCrowdReachableFlowCellSearchResult FCrowdSharedFlowFieldKernel::FindNearestReachableCell(
  const FCrowdSharedFlowField& Field,
  const FVector& Location,
  const int32 MaximumRingDistance)
{
  ...
  FCrowdReachableFlowCellSearchResult Result;
...
FCrowdSharedFlowConstraintResult FCrowdSharedFlowFieldKernel::ConstrainMovement(
  const FCrowdSharedFlowFieldConfig& Config,
  const FVector& Start,
...
{
  return ConstrainMovementCompatible(
    Config, Start, Proposed, FixedStepSeconds, bConstrainToFlowBounds);
}

FCrowdSharedFlowConstraintDiagnostic
FCrowdSharedFlowFieldKernel::DiagnoseMovementConstraint(
  const FCrowdSharedFlowFieldConfig& Config,
  const FVector& Start,
  const FVector& Proposed,
  const bool bConstrainToFlowBounds)
{
  ...
  FCrowdSharedFlowConstraintDiagnostic Result;
  Result.Start = Start;
...
bool FCrowdSharedFlowFieldKernel::IsInsideInflatedObstacle(
  const FCrowdSharedFlowFieldConfig& Config,
  const FVector& Location)
{
