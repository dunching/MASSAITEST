...
[COMPUTED][HIGH] 每个fixed-step在Plan/correction边界处理完成后先构建Runtime基础snapshot；Shared Flow、Target Region与Business分别发布按AgentId排序的不可变Guidance overlay。`FCrowdMassRuntimeBridge::BuildGuidanceRecords`把基础snapshot与三类overlay稳定合并为Compose WORK记录，要求Shared Flow覆盖全部Agent，并拒绝重复provider/Agent、错误revision或错误provider。WORK完成且全量AgentId/result集合验证通过后，GT一次更新Runtime镜像、Demo composed和唯一MoveIntent。Runtime镜像、overlay与基础snapshot都是可重建派生状态，correction rollback后从恢复的Demo事实重新生成，不复制进rollback snapshot。

[COMPUTED][HIGH] Local Predictive不再重新读取Runtime identity/state/properties/composed fragments来构建WORK输入；它直接消费同一boundary基础snapshot与prepared Core composed结果。Runtime WORK调用Core kernel并输出pair、grant、result、summary和可选trace。Mass查询只保留完整结果集合验证以及Runtime/Demo local-velocity原子发布；Demo Pipeline继续保存grant、diagnostic和rollback状态，因此本切片没有改变correction replay语义。
...
[COMPUTED][HIGH] Particle Safety从同一boundary snapshot与prepared MovementPredict结果构造Core agent；`FCrowdMassParticleWork`执行Core Solve和applied-state安全复验，输出candidate/applied summary、hash、pair、result及可选trace。Mass查询保留诊断、路线/稳定指标、T1状态及Runtime/Demo兼容结果发布，不再为WORK基础输入重复读取身份、位置或Particle属性。
...
[COMPUTED][HIGH] 2026-07-22：Development、DebugGame Editor、`git diff --check`、当前102/102项`CrowdDemo`自动化及12/12项`MassCrowd`插件自动化通过。Runtime测试覆盖Target Region四阶段WORK、统一boundary输入、MovementPredict语义、输入反序、重复Agent拒绝及旧/Core/Runtime等价。
...
[COMPUTED][HIGH] 单boundary Gather前九切片已接入：`TryBeginFixedStep()`成功后由专用GT processor一次读取身份、FormationIndex、Formation Radius、RoundSim位置/速度/Yaw、Movement速度上限和Particle半径/间隔/Mobility，构建Runtime `FCrowdMassBoundarySnapshot`及最小Demo formation facts。Flow、Target与Business发布Guidance overlay；Runtime稳定合并后，Compose、Local Predictive、MovementPredict、Particle/Obstacle、Facing与MovementFinalize从snapshot/prepared链构建WORK输入，不再为这些基础事实重复读取Mass fragments。Finalize写入与Demo业务/诊断采集现已成为两个明确processor；后者仍需一次只读Mass遍历，但Flow、Obstacle、Guidance与Facing rollback事实均已改用snapshot/prepared链。
...
[INFERRED][HIGH] 下一步应处理post-finalize仍读取的T1 OpenSpawn状态及Combat/Visual业务事实：先区分rollback必需状态与纯诊断状态，再选择扩展boundary业务快照或建立独立Demo诊断快照。Identity与最终RoundSim state仍是post-finalize对实际提交状态采样的必要事实，不应为追求形式上的“零查询”而复制第二份权威状态。原子预验证和Finalize成功门必须保留；在剩余GT职责和按能力archetype尚未收敛前，不能称整个boundary只读Mass一次。不得把当前20实体证据外推为100/500能力。
