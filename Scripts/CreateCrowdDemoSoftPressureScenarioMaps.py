...
MAPS = [
    ("CrowdDemo_SimRoundSoftPressureSmall", 20),
    ("CrowdDemo_SimRoundSoftPressureMedium", 100),
    ("CrowdDemo_SimRoundSoftPressureCohort", 500),
]
...
def _create_map(map_name, entity_count):
    ...
    unreal.log("CrowdDemoSoftPressureMapCreated path={} agents={}".format(
        package_path, entity_count))


def main():
    unreal.EditorAssetLibrary.make_directory("/Game/Maps")
    selected = os.environ.get("CROWD_DEMO_SOFT_PRESSURE_MAP_INDEX")
    rows = [MAPS[int(selected)]] if selected is not None else MAPS
    for map_name, entity_count in rows:
        _create_map(map_name, entity_count)

    # UE 5.7 commandlet-style editor sessions can continue ticking editor-only
    # actor factories after ExecutePythonScript has completed.  This generator
    # is intentionally one package per process, so exit as soon as the package
    # has been saved instead of leaving a transient editor world alive.
