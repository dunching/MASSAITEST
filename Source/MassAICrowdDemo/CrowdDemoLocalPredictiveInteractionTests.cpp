...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
  FCrowdDemoRollbackDerivedResourceContractTest,
  "CrowdDemo.SF.Rollback.DerivedResourcesAndMutableExecution",
  EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCrowdDemoRollbackDerivedResourceContractTest::RunTest(const FString& Parameters)
{
  UCrowdDemoRoundSimPipelineSubsystem* Pipeline =
    NewObject<UCrowdDemoRoundSimPipelineSubsystem>();
  ...
  Packet.Rules.TargetRegionTransportSettings.bEnabled = 1;
  Pipeline->ActivatePlan(Packet, 2, false);

  FCrowdDemoTargetRegionFlowPlan& Plan = Pipeline->GetPreparedTargetRegionPlan();
  Plan.bValid = true;
  Plan.PlanEpoch = 7;
  Plan.BuildFixedStepIndex = 12;
  Plan.TargetRevision = 3;
  Plan.TransportHash = 0x12345678u;
  FCrowdDemoTargetRegionQuotaExecutionState& Execution =
    Pipeline->GetTargetRegionQuotaExecution();
  Execution.bValid = true;
  Execution.PlanEpoch = 7;
  Execution.PlanTransportHash = Plan.TransportHash;
  Execution.CompletedTransitionCount = 4;

  Pipeline->GetPreparedTargetRegionTopology().bValid = true;
  Pipeline->GetPreparedTargetRegionDemand().bValid = true;
  Pipeline->GetPreparedTargetRegionAgents().AddDefaulted();
  Pipeline->GetPreparedTargetRegionGuidance().AddDefaulted();

  FCrowdDemoTargetRegionCapabilityCohortRuntime Cohort;
  Cohort.Cohort.CapabilityProfileKey = 99u;
  Cohort.Plan = Plan;
  Cohort.Plan.TransportHash = 0x87654321u;
  Cohort.QuotaExecution = Execution;
  Cohort.QuotaExecution.PlanTransportHash = Cohort.Plan.TransportHash;
  Cohort.Topology.bValid = true;
  Cohort.Demand.bValid = true;
  Cohort.Agents.AddDefaulted();
  Cohort.Guidance.AddDefaulted();
  Cohort.SolverMillisecondsSamples = {1.0f};
  Pipeline->GetCapabilityCohorts() = {Cohort};

  Pipeline->RecordSoftPressureRollbackSnapshot(
    12, TArray<FCrowdDemoSoftPressureRollbackAgentState>());
  const FCrowdDemoSoftPressureRollbackSnapshot* Snapshot =
    Pipeline->FindSoftPressureRollbackSnapshot(12);
  TestNotNull(TEXT("rollback snapshot exists"), Snapshot);
  if (!Snapshot) return false;
  TestTrue(TEXT("homogeneous plan is referenced as immutable resource"),
    Snapshot->TargetRegionPlanResourceKey != 0);
  TestEqual(TEXT("snapshot stores compact cohort execution state"),
    Snapshot->TargetRegionCapabilityCohorts.Num(), 1);
  TestTrue(TEXT("cohort plan is referenced as immutable resource"),
    Snapshot->TargetRegionCapabilityCohorts[0].PlanResourceKey != 0);
  ...
  Pipeline->GetPreparedTargetRegionAgents().AddDefaulted();
  Pipeline->GetPreparedTargetRegionGuidance().AddDefaulted();
  Pipeline->GetCapabilityCohorts()[0].SolverMillisecondsSamples.Add(2.0f);
  Pipeline->RestoreSoftPressureRuntime(*Snapshot);

  TestTrue(TEXT("immutable homogeneous plan restored by resource key"),
    Pipeline->GetPreparedTargetRegionPlan().bValid);
  TestEqual(TEXT("homogeneous plan hash restored"),
    Pipeline->GetPreparedTargetRegionPlan().TransportHash, 0x12345678u);
  TestEqual(TEXT("mutable quota execution restored"),
    Pipeline->GetTargetRegionQuotaExecution().CompletedTransitionCount, 4);
  TestFalse(TEXT("topology is derived and invalidated on restore"),
    Pipeline->GetPreparedTargetRegionTopology().bValid);
  TestFalse(TEXT("demand is derived and invalidated on restore"),
    Pipeline->GetPreparedTargetRegionDemand().bValid);
  TestEqual(TEXT("derived transport agents are not copied by snapshot"),
    Pipeline->GetPreparedTargetRegionAgents().Num(), 0);
  TestEqual(TEXT("derived guidance is not copied by snapshot"),
    Pipeline->GetPreparedTargetRegionGuidance().Num(), 0);

  TestEqual(TEXT("capability cohort restored"),
    Pipeline->GetCapabilityCohorts().Num(), 1);
  if (Pipeline->GetCapabilityCohorts().Num() == 1)
  {
    const FCrowdDemoTargetRegionCapabilityCohortRuntime& Restored =
      Pipeline->GetCapabilityCohorts()[0];
    TestEqual(TEXT("cohort immutable plan hash restored"),
      Restored.Plan.TransportHash, 0x87654321u);
    TestTrue(TEXT("cohort topology is derived"), !Restored.Topology.bValid);
    TestTrue(TEXT("cohort demand is derived"), !Restored.Demand.bValid);
    TestEqual(TEXT("cohort derived agents are absent"), Restored.Agents.Num(), 0);
    TestEqual(TEXT("cohort derived guidance is absent"), Restored.Guidance.Num(), 0);
    TestEqual(TEXT("solver samples restore by cursor, not zero fill"),
      Restored.SolverMillisecondsSamples.Num(), 1);
    if (Restored.SolverMillisecondsSamples.Num() == 1)
      TestEqual(TEXT("solver sample value preserved"),
        Restored.SolverMillisecondsSamples[0], 1.0f);
  }
  return true;
}
