...
float RequiredSeparation(
  const FCrowdDemoLocalPredictiveAgent& A,
  const FCrowdDemoLocalPredictiveAgent& B)
{
...
bool BuildPairHalfPlane(
  const FCrowdDemoLocalPredictiveAgent& Agent,
  const FCrowdDemoLocalPredictiveAgent& Other,
  const float Responsibility,
  const FCrowdDemoLocalPredictiveSettings& Settings,
  const int32 StableOrder,
  FCrowdDemoVelocityHalfPlane& OutHalfPlane)
{
...
bool IsInsideObstacle(
  const FCrowdDemoLocalPredictiveAgent& Agent,
  const FCrowdDemoSharedFlowObstacleSpec& Obstacle,
  const FVector2f Position)
{
  const float Clearance = Agent.PhysicalRadiusCm + Agent.HardSafetyGapCm;
...
void AddBoundsConstraints(
  ...
  const FCrowdDemoSharedFlowFieldConfig& FlowConfig,
  const FCrowdDemoLocalPredictiveSettings& Settings,
  TArray<FCrowdDemoVelocityHalfPlane>& InOutHalfPlanes)
{
  const float Clearance = Agent.PhysicalRadiusCm + Agent.HardSafetyGapCm;
  const float InverseStep = 1.0f / FMath::Max(Settings.FixedStepSeconds, 0.001f);
...
FCrowdDemoVelocityHalfPlane BuildEnvironmentHalfPlane(
  const FCrowdDemoLocalPredictiveAgent& Agent,
  ...
  const int32 StableOrder)
{
  ...
  FCrowdDemoVelocityHalfPlane HalfPlane;
  ...
  int32& OutEnvironmentConstraintCount)
{
  ...
  if (!FCrowdDemoVelocityHalfPlaneKernel::SolveContinuous(Input, InOutVelocity))
    return false;
...
bool FindComponentCommonVelocity(
  const TConstArrayView<FCrowdDemoLocalPredictiveAgent> Agents,
  ...
  int32& OutEnvironmentConstraintCount)
{
  ...
  for (const int32 AgentIndex : AgentIndices)
  {
    ...
    const FCrowdDemoLocalPredictiveAgent& Agent = Agents[AgentIndex];
  ...
  FCrowdDemoVelocityHalfPlaneResult SolveResult =
    FCrowdDemoVelocityHalfPlaneKernel::Solve(Input);
  TSet<uint64> EnvironmentKeys;
  ...
  for (int32 Pass = 0; SolveResult.bQuantizedValid && Pass < MaximumPasses; ++Pass)
  {
    ...
    ++OutEnvironmentConstraintCount;
    SolveResult = FCrowdDemoVelocityHalfPlaneKernel::Solve(Input);
  }
  return false;
}

bool IsValidAgentInput(const FCrowdDemoLocalPredictiveAgent& Agent)
{
...
int32 ProgressDeficit(const FCrowdDemoLocalPredictiveAgent& Agent)
{
  const float Requested = Agent.PreferredVelocity.Size();
...
void FCrowdDemoLocalPredictiveInteractionKernel::BuildCandidatePairs(
  const TConstArrayView<FCrowdDemoLocalPredictiveAgent> Agents,
  const FCrowdDemoLocalPredictiveSettings& Settings,
  TArray<FCrowdDemoLocalPredictivePair>& OutPairs)
{
  ...
  for (const uint64 Key : SortedKeys)
  {
    ...
    FCrowdDemoLocalPredictivePair& Pair = OutPairs.AddDefaulted_GetRef();
...
bool FCrowdDemoLocalPredictiveInteractionKernel::ValidateJointResult(
  const TConstArrayView<FCrowdDemoLocalPredictiveAgent> Agents,
  const FCrowdDemoSharedFlowFieldConfig& FlowConfig,
  const FCrowdDemoLocalPredictiveSettings& Settings,
  const TConstArrayView<FCrowdDemoLocalPredictivePair> ConflictPairs,
  const TConstArrayView<FCrowdDemoLocalPredictiveResult> Results)
{
...
void FCrowdDemoLocalPredictiveInteractionKernel::Solve(
  const TConstArrayView<FCrowdDemoLocalPredictiveAgent> Agents,
  const FCrowdDemoSharedFlowFieldConfig& FlowConfig,
  const FCrowdDemoLocalPredictiveSettings& Settings,
  const TConstArrayView<FCrowdDemoLocalPredictiveGrantState> PreviousGrantStates,
  ...
  FCrowdDemoLocalPredictiveDiagnosticTrace* OutTrace)
{
  OutConflictPairs.Reset();
  OutGrantStates.Reset();
  OutResults.Reset();
  OutSummary = FCrowdDemoLocalPredictiveSummary();
  if (OutTrace) *OutTrace = FCrowdDemoLocalPredictiveDiagnosticTrace();
  TArray<FCrowdDemoLocalPredictiveAgent> SortedAgents(Agents);
  ...
  for (int32 Root = 0; Root < SortedAgents.Num(); ++Root)
  {
    ...
    const FCrowdDemoLocalPredictiveGrantState* Previous = nullptr;
  ...
  for (int32 AgentIndex = 0; AgentIndex < SortedAgents.Num(); ++AgentIndex)
  {
    ...
    auto SolveResult = FCrowdDemoVelocityHalfPlaneKernel::Solve(Input);
    TSet<uint64> EnvironmentKeys;
  ...
  const auto IsPairSafe = [&](const FCrowdDemoLocalPredictivePair& Pair)
  {
    if (!OutResults.IsValidIndex(Pair.MinAgentIndex)
      || !OutResults.IsValidIndex(Pair.MaxAgentIndex)) return false;
    const FCrowdDemoLocalPredictiveResult& ResultA = OutResults[Pair.MinAgentIndex];
    const FCrowdDemoLocalPredictiveResult& ResultB = OutResults[Pair.MaxAgentIndex];
  ...
  {
    const FCrowdDemoLocalPredictiveAgent& Agent = SortedAgents[AgentIndex];
    FCrowdDemoLocalPredictiveResult& Result = OutResults[AgentIndex];
    ...
    TArray<FCrowdDemoVelocityHalfPlane> HalfPlanes;
    int32 PairOrder = 0;
    for (const FCrowdDemoLocalPredictivePair& Pair : OutConflictPairs)
    {
      const bool bMin = Pair.MinAgentIndex == AgentIndex;
      const bool bMax = Pair.MaxAgentIndex == AgentIndex;
      if (!bMin && !bMax) continue;
  ...
  for (int32 CompletionPass = 0; CompletionPass <= Candidates.Num(); ++CompletionPass)
  {
    ...
    for (const FCrowdDemoLocalPredictivePair& Candidate : Candidates)
    {
      ...
      FCrowdDemoLocalPredictivePair Added = Candidate;
  ...
  // toward one common velocity, and is accepted only after environment checks.
  {
    ...
    for (int32 Root = 0; Root < SortedAgents.Num(); ++Root)
    {
      ...
      for (const FCrowdDemoLocalPredictivePair& Pair : Candidates)
      ...
      const FVector2f AppliedTranslation = Quantize(
        ...
        Settings.VelocityQuantumCmps);
      FCrowdDemoLocalPredictiveComponentTrace ComponentTrace;
      for (const int32 AgentIndex : ComponentIndices)
      {
  ...
  // and improves the component objective; it never introduces a minimum speed.
  {
    ...
    for (int32 Root = 0; Root < SortedAgents.Num(); ++Root)
    {
      ...
      TArray<FCrowdDemoLocalPredictiveResult> RecoveryResults = OutResults;
      double BaselineObjective = 0.0;
      for (const int32 AgentIndex : ComponentIndices)
      {
        ...
        Iteration < FMath::Max(1, Settings.JointIterationCount); ++Iteration)
      {
        for (const FCrowdDemoLocalPredictivePair& Candidate : Candidates)
        {
          ...
          if (const FCrowdDemoLocalPredictivePair* Existing =
      ...
      FCrowdDemoLocalPredictiveComponentTrace RecoveryTrace;
      for (const int32 AgentIndex : ComponentIndices)
      {
      ...
      if (OutTrace)
      {
        const int32 ExistingTraceIndex = OutTrace->Components.IndexOfByPredicate(
          [&](const FCrowdDemoLocalPredictiveComponentTrace& Existing)
    ...
    for (int32 Root = 0; Root < SortedAgents.Num(); ++Root)
    {
        ...
        Iteration < FMath::Max(1, Settings.JointIterationCount); ++Iteration)
      {
        for (const FCrowdDemoLocalPredictivePair& Pair : OutConflictPairs)
        {
          if (!ComponentIndices.Contains(Pair.MinAgentIndex)
            || !ComponentIndices.Contains(Pair.MaxAgentIndex)) continue;
          FCrowdDemoVelocityHalfPlane MinHalfPlane;
          FCrowdDemoVelocityHalfPlane MaxHalfPlane;
...
bool FCrowdDemoLocalPredictiveInteractionKernel::BuildComponentFixture(
  ...
  const TConstArrayView<FCrowdDemoLocalPredictiveGrantState> PreviousGrantStates,
  const TConstArrayView<FCrowdDemoLocalPredictivePair> ConflictPairs,
  const TConstArrayView<FCrowdDemoLocalPredictiveGrantState> GrantStates,
  const TConstArrayView<FCrowdDemoLocalPredictiveResult> Results,
...
{
  OutFixture = FCrowdDemoLocalPredictiveComponentFixture();
  ...
  {
    bExpanded = false;
    for (const FCrowdDemoLocalPredictivePair& Pair : ConflictPairs)
    {
  ...
  for (const FCrowdDemoLocalPredictiveComponentTrace& Component : Trace.Components)
  {
  ...
  for (const auto& Pair : ConflictPairs)
  {
    ...
    if (!MinIndex || !MaxIndex) continue;
    FCrowdDemoLocalPredictivePair Copy = Pair;
  ...
  const auto FoldResult = [&](const FCrowdDemoLocalPredictiveResult& Result)
  {
    Hash = HashInt(Hash, Result.AgentId);
